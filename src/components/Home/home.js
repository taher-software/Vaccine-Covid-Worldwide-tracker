...
import { globalRateVaccination } from "../../Logic/Logic";
...
import { countryRateVaccination } from "../../Logic/Logic";
    ...
    <div className="general-inf">
      ...
      <div className="worldwide-vacc">
        <div className="display-text">
          <h1>WORLDWIDE</h1>
          <span> {worldVaccination ==='Waiting data...'? worldVaccination: `${worldVaccination}% vaccinated`}</span>
        </div>
      </div>
    </div>
    ...
    <Grid container columns={12} style={{ backgroundColor: 'rgb(85, 136, 227)' }}>
      {countries.map((country) =>{
        if (country.toLowerCase() !== 'global'){
          ...
          i += 1;
          return(
           <Grid item xs={6} key={country}  style={{ 
           height:'242px',
           display: 'flex',
           flexDirection: 'column',
           backgroundColor: `${backColor[i%4]}`,
           }}
           onClick= {(e) => getDetails(country)}
           >
           ...
           <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', justifyContent: 'center', height: '98px', marginRight: '2.5%'}}>
             <Typography variant="h2" style={{ textTransform: 'uppercase', fontFamily:'Gill, sans-serif', fontSize: '14px', color: 'white' }}>{country}</Typography>
             <Typography style={{ fontFamily:'Lato, sans-serif', color:'white' }} variant="span" >{rate==='Waiting data...'? rate: rate === 'No data available...'? rate:`${rate}% vaccinated`}</Typography>
           </div>
         </Grid>);
        }
      })}
